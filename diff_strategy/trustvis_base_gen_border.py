...
for iteration in range(EPOCH_START, EPOCH_END+EPOCH_PERIOD, EPOCH_PERIOD):
    ...
    spatial_cons = TrustvisSpatialEdgeConstructor(data_provider, iteration, S_N_EPOCHS, B_N_EPOCHS, N_NEIGHBORS, pred_lambda, None, fin_gen_border_data)
    ...
    # pred = data_provider.get_pred(iteration, feature_vectors)
    # dataset = TrustDataHandler(edge_to, edge_from, feature_vectors, attention, pred)
    ...
    # trainer = TempoALTrainer(model, criterion, optimizer, lr_scheduler, edge_loader=edge_loader, DEVICE=DEVICE,iteration=iteration, data_provider=data_provider, prev_model=prev_model, S_N_EPOCHS=S_N_EPOCHS, B_N_EPOCHS=B_N_EPOCHS, N_NEIGHBORS=N_NEIGHBORS,threshold=1,resolution=400, mul=1, alpha=pred_lambda, gen_border_data=gen_border_data)
