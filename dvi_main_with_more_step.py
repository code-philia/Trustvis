...
DECODER_DIMS_LIST = [[256,256,512],[128,256,256],[128,64,2]]
...
start_flag = 1
# model = VisModel(ENCODER_DIMS[0], DECODER_DIMS[0])
# prev_model = VisModel(ENCODER_DIMS[0], DECODER_DIMS[0])
...
for iteration in range(EPOCH_START, EPOCH_END+EPOCH_PERIOD, EPOCH_PERIOD):
    ...
    for layer in range(2,len(ENCODER_DIMS_LIST)):

        model = VisModel(ENCODER_DIMS_LIST[layer], ENCODER_DIMS_LIST[layer])
        t0 = time.time()

        if start_flag:
            spatial_cons = SingleEpochSpatialEdgeConstructorLEVEL(data_provider, iteration, S_N_EPOCHS, B_N_EPOCHS, N_NEIGHBORS,None)
            start_flag = 0
        else:
            prev_model = VisModel(ENCODER_DIMS[layer-1], DECODER_DIMS[layer-1])
            projector = DVIProjector(vis_model=prev_model, content_path=CONTENT_PATH, vis_model_name="{}_{}".format(VIS_MODEL_NAME,layer-1), device=DEVICE)
            spatial_cons = SingleEpochSpatialEdgeConstructorLEVEL(data_provider, iteration, S_N_EPOCHS, B_N_EPOCHS, N_NEIGHBORS,projector)
        ...
        trainer.save(save_dir=save_dir, file_name="{}_{}".format(VIS_MODEL_NAME,layer))
