...
class visualizer(VisualizerAbstractClass):
    ...
    def get_epoch_plot_measures(self, epoch):
        ...
        for i in range(len(self.projector)):
             data = self.projector[i].batch_project(epoch, data)
    ...
    def get_epoch_decision_view(self, epoch, resolution, xy_limit=None, forDetail=False):
        ...
        for i in range(len(self.projector)):
            ...
            grid_C = self.projector[len(self.projector) - i-1].batch_inverse(epoch, grid_C)
            print("grid",grid_C.shape)
    ...
    def savefig(self, epoch, path="vis"):
        ...
        train_data_c = self.data_provider.train_representation(epoch)
        for i in range(len(self.projector)):
             train_data_c = self.projector[i].batch_project(epoch, train_data_c)
        
        embedding = train_data_c
...
class DenseALvisualizer(visualizer):
    ...
    def get_epoch_decision_view(self, iteration, epoch, resolution):
        ...
        for i in range(len(self.projector)):
            ...
            grid = self.projector[len(self.projector) - i -1].batch_inverse(epoch, grid)
