...
class visualizer(VisualizerAbstractClass):
    ...
    def get_epoch_plot_measures(self, epoch):
        ...
        data = np.load(os.path.join(self.data_provider.model_path, 'Epoch_{}'.format(epoch),'adv_rep.npy'))
    ...
    def savefig(self, epoch, path="vis"):
        ...
        train_data = np.load(os.path.join(self.data_provider.model_path, 'Epoch_{}'.format(epoch),'adv_rep.npy'))[indices]
        train_data = train_data.reshape(train_data.shape[0],train_data.shape[1])
        train_labels = self.data_provider.train_labels(epoch)[indices]
        ...
        for c in range(self.class_num):
            data = embedding[np.logical_and(train_labels == c, True )]
