...
class visualizer(VisualizerAbstractClass):
    def __init__(self, data_provider, projector, resolution, new_pred,cmap='tab10'):
    ...
    def _init_plot(self, only_img=False):
        ...
        for c in range(self.class_num):
            ...
            plot = self.ax.plot([], [], '.', label=self.classes[c], ms=8,
                color=color, zorder=1, picker=mpl.rcParams['lines.markersize'])
        ...
        #         fillstyle='full', ms=5, mew=2.5, zorder=4)
        #     self.sample_plots.append(plot[0])
        for c in range(self.class_num):
            color = self.cmap(c / (self.class_num - 1))
            plot = self.ax.plot([], [], '.', markeredgecolor=color,
                                fillstyle='full', ms=8, zorder=5)
            self.sample_plots.append(plot[0])
        ...
        for c in range(self.class_num):
            color = self.cmap(c / (self.class_num - 1))
            plot = self.ax.plot([], [], '.', markeredgecolor=color,
                                fillstyle='full', ms=6, zorder=5)
            self.sample_plots.append(plot[0])
        
        # Initialize white border points
        for c in range(self.class_num):
            color = self.cmap(c / (self.class_num - 1))
            plot = self.ax.plot([], [], '.', label="border", ms=14,
                    color=color, markeredgecolor=color, zorder=12, picker=mpl.rcParams['lines.markersize'])
            self.sample_plots.append(plot[0])
        
        for c in range(self.class_num):
            color = self.cmap(c/(self.class_num-1))
            plot = self.ax.plot([], [], 'o', markeredgecolor='yellow',
                fillstyle='full', ms=9, mew=3, zorder=2)
            self.sample_plots.append(plot[0])
    ...
    def show_grid_embedding(self, epoch, data, embedding, border, label, pred, noOutline=False, path="vis"):
        '''
        Shows the current plot.
        '''
        self._init_plot(only_img=True)
        ...
        pred = pred
        # n = len(self.new_pred)
        # pred[-n:] = self.new_pred
        # pred = np.append(pred,self.new_pred)
        ...
        if noOutline == True:
            ...
            for c in range(self.class_num):
                ...
                self.sample_plots[3*self.class_num+ c].set_data(data.transpose())
            for c in range(self.class_num):
                data = embedding[np.logical_and(train_labels == c, border==3)]
