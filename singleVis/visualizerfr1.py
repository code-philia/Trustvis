...
class visualizer(VisualizerAbstractClass):
    def __init__(self, data_provider, projector, resolution,autoencoder, cmap='tab10'):
    ...
    def get_epoch_plot_measures(self, epoch):
        ...
        data = self.autoencoder.encoder(torch.Tensor(self.data_provider.train_representation(19))).cpu().detach().numpy()
    ...
    def get_epoch_decision_view(self, epoch, resolution, xy_limit=None, forDetail=False):
        ...
        grid_samples = self.autoencoder.decoder(torch.Tensor(grid_samples)).cpu().detach().numpy()
        print("grid_samples",grid_samples.shape)

        mesh_preds = self.data_provider.get_pred(19, grid_samples)
    ...
    def savefig(self, epoch, path="vis", th=0.15):
        '''
        ...
        train_data = self.data_provider.train_representation(19)
        train_labels = self.data_provider.train_labels(19)
        pred = self.data_provider.get_pred(5, train_data)
        pred = pred.argmax(axis=1)

        embedding = self.projector.batch_project(epoch, self.autoencoder.encoder(torch.Tensor(train_data)).cpu().detach().numpy())
        ...
        mesh_preds = self.data_provider.get_pred(epoch, train_data)
        mesh_preds = mesh_preds + 1e-8
