...
class visualizer(VisualizerAbstractClass):
    def __init__(self, data_provider, test_representation, projector, resolution, indicates, cmap='tab10'):
    ...
    #         plot = self.ax.plot([], [], '.', label=self.classes[c], ms=9,
    #             color=color, zorder=3, mew=3, picker=mpl.rcParams['lines.markersize'])
    #         self.sample_plots.append(plot[0])

    #     # labels != prediction, labels be a large circle
    #     for c in range(self.class_num):
    #         color = self.cmap(c/(self.class_num-1))
    #         plot = self.ax.plot([], [], '.', color=color,
    #             fillstyle='full', ms=9, mew=3.5, zorder=3)
    #         self.sample_plots.append(plot[0])
    ...
    def _init_plot(self, only_img=False):
        ...
        for c in range(self.class_num):
            color = self.cmap(c / (self.class_num - 1))
            plot = self.ax.plot([], [], 'o', label="border", ms=9, mew=2.5,
                    fillstyle='none', color="yellow", markeredgecolor='yellow', zorder=1, picker=mpl.rcParams['lines.markersize'])
            self.sample_plots.append(plot[0])
    ...
    def get_epoch_decision_view(self, epoch, resolution):
        ...
        logits = torch.tensor(mesh_preds, dtype=torch.float32)
        labels = torch.tensor(mesh_preds.argmax(axis=1), dtype=torch.long)

        temperature = self.compute_temperature(logits, labels)
        print("temperature",temperature)
        temperature = 3
        # 使用得到的温度来校准你的预测分数
        calibrated_logits = logits / temperature
        mesh_preds = torch.nn.functional.softmax(calibrated_logits, dim=-1)

        mesh_preds = mesh_preds.detach().numpy()
        print("mesh_preds",mesh_preds.shape)
    ...
    def savefig(self, epoch, path="vis"):
        ...
        after_trans = self.test_representation
        if len(self.indicates):
            ...
            after_trans = self.test_representation[self.indicates]
    ...
    def savefig_cus(self, epoch, data, pred, labels, path="vis"):
        ...
        for c in range(self.class_num):
            data = embedding[np.logical_and(labels == pred)]
