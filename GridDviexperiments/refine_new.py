...
vis = visualizer(data_provider, projector, 200, "tab10")
grid_high, grid_emd ,border = vis.get_epoch_decision_view(epoch,800,None, True)
...
optimizer = torch.optim.Adam(pre_model.parameters(), lr=.0005, weight_decay=1e-5)
...
border_elements = dataGeneration.get_near_epoch_border(20)
...
class DVIReFineTrainer(SingleVisTrainer):
    ...
    def train(self, PATIENT, MAX_EPOCH_NUMS):
        patient = PATIENT
        print("patient",patient,MAX_EPOCH_NUMS)
    ...
    def train_step(self):
        ...
        recoverposition_losses.append(pos_loss.mean()) # remove .item() here
        # ===================backward====================
        recoverposition_loss = torch.stack(recoverposition_losses).mean()

        loss_new = recoverposition_loss
        ...
        self.model.eval()
        print('umap:{:.4f}\trecon_l:{:.4f}\ttemporal_l:{:.4f}\tloss:{:.4f}\tecoverposition_losses:{}'.format(0,
                                                                0,
                                                                0,
                                                                0 ,loss_new ))
        return self.loss
...
data_ = np.concatenate((data_provider.train_representation(epoch), data_provider.border_representation(epoch),grid_high), axis=0)
trainer = DVIReFineTrainer(pre_model, criterion, optimizer, lr_scheduler , [],DEVICE, data_, disable_encoder_grad = True)

t2=time.time()
trainer.train(200, 200)
t3 = time.time()
